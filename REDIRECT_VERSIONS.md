...
2. Mở http://localhost:3000/redirect.html (hoặc port mà Vite báo)
...
- `public/redirect.html` – Form chỉ submit khi URL có `gclid` (Google Ads)
...
1. Xóa `public/redirect.html`
2. Xóa thư mục `api/`
3. Cập nhật link trên site từ `/redirect.html` về affiliate link trực tiếp
4. Commit và deploy

### Bật lại v1 (dùng redirect)

1. Sao chép `_versions/redirect-v1/public/redirect.html` → `public/redirect.html`
2. Sao chép `_versions/redirect-v1/api/redirect.ts` → `api/redirect.ts`
3. Cập nhật link trên site trỏ tới `/redirect.html` (nếu cần)
4. Commit và deploy
