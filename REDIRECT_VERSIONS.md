...
2. Mở http://localhost:3000/register/ (hoặc port mà Vite báo)
...
- `public/register/index.html` – Form chỉ submit khi URL có `gclid` (Google Ads)
...
1. Xóa thư mục `public/register/`
2. Xóa thư mục `api/`
3. Cập nhật link trên site từ `/register/` về affiliate link trực tiếp
4. Commit và deploy

### Bật lại v1 (dùng redirect)

1. Sao chép `_versions/redirect-v1/public/register/` → `public/register/`
2. Sao chép `_versions/redirect-v1/api/redirect.ts` → `api/redirect.ts`
3. Cập nhật link trên site trỏ tới `/register/` (nếu cần)
4. Commit và deploy
